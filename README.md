...
* **완벽한 반응형 디자인**: 모바일(480px), 태블릿(768px), 데스크톱(1024px+)까지 모든 디바이스에 최적화
* **모바일 햄버거 메뉴**: 768px 이하 화면에서 우측 슬라이드 패널 메뉴로 자동 전환
...
**반응형 레이아웃:**
- **데스크톱 (1024px 이상)**: 풀 레이아웃, 4개 컬럼 그리드, 좌우 메뉴 섹션
- **태블릿 (768px - 1023px)**: 3개 컬럼 그리드, 간격 조정, 햄버거 메뉴 전환
- **모바일 (480px - 767px)**: 1-2열 세로 배치, 메뉴 섹션 세로 정렬, 이미지 높이 축소
- **소형 모바일 (480px 이하)**: 1열 레이아웃, 폰트 크기 축소, 최소 간격 유지
...
#### 1. 네비게이션
* **데스크톱**: 가로 메뉴바 형태 (Activity, Certificate, Project, Resume)
* **모바일 (768px↓)**: 
  - 햄버거 메뉴 아이콘 (삼선 버튼)
  - 우측 슬라이드 패널 메뉴 (70% width, 최대 300px)
  - 메뉴 항목 세로 배치, 흰색 텍스트
  - 외부 영역 클릭 시 자동 닫힘
  - 배경 오버레이 효과
...
#### 2. 홈페이지 섹션
* **메뉴 섹션**: 
  - 데스크톱: 이미지(50%) + 텍스트(50%) 좌우 배치
  - 모바일: 이미지(35vh) + 텍스트 세로 배치, 스크롤 최적화
* **Footer**: 
  - 데스크톱: 4개 컬럼 (CONTACT, EDUCATION, LOCATION, CONNECT)
  - 모바일: 2개 컬럼만 표시 (CONTACT, CONNECT), 로고 위 좌우 배치
...
#### 4. Project 페이지
* **모달 창**: 모든 디바이스에서 스크롤 가능, 배경 스크롤 방지
* **그리드**: 데스크톱 3열 → 태블릿 2열 → 모바일 1열

#### 5. Activity 페이지
* **타임라인**: 모바일에서 왼쪽 정렬, 간격 축소
* **카드 레이아웃**: 세로 스크롤 최적화

#### 6. Resume 페이지
* **PDF 뷰어**: 모든 화면 크기에서 반응형으로 크기 조정
* **컨테이너**: 모바일에서 패딩 축소
...
* 스크롤 최상단 버튼 추가
* **모바일 반응형 디자인 구현**:
  - 4개 브레이크포인트 (1200px, 1024px, 768px, 480px) 적용
  - 햄버거 메뉴 시스템 개발 (슬라이드 패널, 오버레이)
  - 모든 페이지 레이아웃 모바일 최적화
  - 푸터 레이아웃 조정 (데스크톱 4컬럼 → 모바일 2컬럼)
  - 섹션 높이 및 스크롤 동작 최적화
